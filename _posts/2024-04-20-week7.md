...
if 조건문 : <br>     수행할 문장 1<br>수행할 문장 2<br>...<br>else : <br> 수행할 문장 a<br>수행할 문장 b<br> ... <br><ul>조건문을 판단해서 참이면 if문 바로 다음 문장(수행할 문장1, 2)을 수행하고, 거짓이면 else문 다음 문장(수행할 문장 a, b)을 수행하게 됩니다. 따라서 else문은 if문 없이 독립적으로 사용할 수 없습니다.</ul><br><ul>if 조건문과 else문 뒤에는 반드시 콜론(:)이 붙습니다. 콜론(:)을 써주지 않으면 문법 오류가 발생합니다.</ul><br><ul>if 조건문 바로 아래 문장부터 if문에 속하는 모든 문장에 들여쓰기를 해줘야 합니다. else문 또한 마찬가지 입니다. 들여쓰기는 코드의 기독성을 높여주며 해당 코드가 어디에 소속되는지 알려주는 약속이기 때문에 지켜주지 않으면 에러가 발생합니다. 들여쓰기 방법은 공백(space bar) 2칸, 4칸, 탭(tab)등 다양합니다. 다만, 들여쓰기를 할 때는 같은 블록 내에서는 들여쓰기 칸수를 동일하게 지켜줘야 하며, 공백과 탭을 섞어서 사용하지 않아야 합니다.</ul>
...
| if 조건문 : <br>     수행할 문장 1<br>수행할 문장 2<br>...<br>elif : <br> 수행할 문장 a<br>수행할 문장 b<br> ...<br>else : <br> 수행할 문장 A<br>수행할 문장 B<br> ...<br><ul>위의 if문부터 조건을 하나씩 판단합니다. 조건문이 참일 경우 해당 명령어들을 수행하고 if문을 종료합니다.</ul><br><ul>예를 들어 첫 번째 if문이 참이라면 수행할 문장 1, 2를 수행하고 종료합니다.</ul><br><ul>하지만 첫 번째 if문이 거짓이면 다음 elif문의 조건을 판단합니다. 여기서 참이면 수행할 문장 a, b를 수행하고 종료하지만, 거짓이라면 다음 조건으로 넘어갑니다. elif는 개수의 제한 없이 사용할 수 있습니다.</ul><br><ul>else는 위의 조건이 모두 거짓일 경우에 사용되는 것으로 필요하다면 사용하고 필요없으면 생략해도 됩니다.
